...
public class BestandshistorieService {
    ...
    public List<ArtikelHistorie> suchBestandshistorie(int artNr, int tage, Boolean istKaufFilter) throws ArtikelNichtGefundenException {
        ...
        return artikel.getBestandshistorie().stream()
                .filter(
                        bestandshistorie -> bestandshistorie
                                .getDatum()
                                .isAfter(bestandshistorie.getDatum().minus(neueTage, ChronoUnit.DAYS))
                ).filter(
